...
export const configureBookingController = (app: Express, dependencies: Dependencies) => {

    const {bookingRepository, uberRepository, tripTypeScanner, transactionPerformer} = dependencies;

    app.get('/customers/:customerId/bookings', async (req, res) => {
        const bookings = await new RetrieveCustomerBookingsHistory(bookingRepository).retrieve(req.params['customerId']);
    ...
    app.post('/customers/:customerId/bookings/:bookingId', async (req, res) => {
        ...
        await new BookUberCommandHandler(
            bookingRepository, uberRepository, tripTypeScanner, transactionPerformer)
            .handle({
                customerId,
                bookingId,
                startPoint,
